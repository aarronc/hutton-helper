...
ITEM_LOOKUP = {"advancedcatalysers" : "Advanced Catalysers",
   ...
   "medicaldiagnosticequipment" : "Medical Diganostic Equipment",
   ...
   "polymers" : "Polymers",
   "powerconverter" : "Power Converters",
...
def journal_entry(cmdr, is_beta, system, station, entry, state):
    ...
    if event == 'MarketBuy':
        ...
        this.status['text'] = "{:,.0f} {} bought".format(float(entry['Count']), ITEM_LOOKUP.get(entry['Type']))

    elif event == 'MarketSell':
        this.status['text'] = "{:,.0f} {} sold".format(float(entry['Count']), ITEM_LOOKUP.get(entry['Type']))
